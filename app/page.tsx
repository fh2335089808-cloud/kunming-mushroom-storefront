...
const products = [
  ...
  { name: '青头菌', source: '昆明木水花市场鲜选', feature: '肉质紧实，清甜回甘', cook: '适合清炒、蒸蛋、煮汤', price: '今日到货后现价，预计 ¥88–¥128 / 500g', image: '/images/mushrooms/greenhead.jpg', availability: '供应充足' },
  { name: '松茸', source: '香格里拉高山到货', feature: '香气清冽，风味细腻', cook: '适合黄油煎、炖汤、蒸饭', price: '今日参考价：¥399 / 500g（以当天到货为准）', image: '/images/mushrooms/matsutake.jpg', availability: '需要预订' },
  { name: '牛肝菌', source: '云南菌市当日鲜选', feature: '菌肉厚实，香气醇厚', cook: '适合爆炒、炒饭、意面', price: '今日到货后现价，预计 ¥108–¥148 / 500g', image: '/images/mushrooms/porcini.jpg', availability: '随市场更新' },
];

const trust = [['云南产区鲜选', '根据菌季和当天状态进行筛选'], ['每日市场更新', '价格、品种和库存随市场变化'], ['昆明同城配送', '具体配送范围和时效微信确认']];
const gallery = [['松茸', '/images/mushrooms/matsutake.jpg'], ['牛肝菌', '/images/mushrooms/porcini.jpg'], ['鸡枞菌', '/images/mushrooms/jizong.jpg'], ['鸡油菌', '/images/mushrooms/chanterelle.jpg'], ['大红菌', '/images/mushrooms/red-mushroom.jpg'], ['青头菌', '/images/mushrooms/greenhead.jpg'], ['干巴菌', '/images/mushrooms/termite-mushroom.jpg'], ['竹荪', '/images/mushrooms/bamboo-fungus.jpg']];
const recipes = [
  { name: '鲜菌海味小炒', note: '鲜菌搭配时蔬与虾仁，旺火快炒，保留山野清鲜。', image: '/images/mushrooms/seafood-mushroom.jpg' },
  { name: '山野菌焖锅', note: '多种鲜菌慢火焖香，适合与家人分享的一餐。', image: '/images/mushrooms/mushroom-pot.jpg' },
];
...
export default function Home() {
  return <main className="overflow-x-hidden bg-[#f7f4ec]">
    <section id="home" className="relative min-h-[76svh] overflow-hidden bg-forest-900 text-white sm:min-h-[720px]">
      <Image priority fill sizes="100vw" src="https://images.unsplash.com/photo-1448375240586-882707db888b?auto=format&fit=crop&w=2000&q=90" alt="云南山野森林" className="object-cover object-[58%_center] opacity-55 sm:object-center" />
      <div className="absolute inset-0 bg-gradient-to-b from-black/20 via-forest-900/15 to-forest-900" />
      <nav className="container relative flex items-center justify-between gap-3 py-4 sm:py-6">
        <a href="#home" className="shrink-0 font-serif text-xl tracking-[.18em] sm:text-2xl">菌鲜到</a>
        <div className="hidden items-center gap-6 text-sm text-white/80 md:flex"><a href="#home">首页</a><a href="#market">今日行情</a><a href="#recipes">鲜菌做法</a><a href="#story">鲜菌故事</a></div>
        <a href="#wechat" className="shrink-0 rounded-full border border-white/50 px-3.5 py-2 text-xs sm:text-sm">微信咨询</a>
      </nav>
      <div className="container relative pb-10 pt-10 sm:pt-24">
        ...
        <div className="mt-4 max-w-2xl space-y-1.5 border-l border-amber-100/45 pl-3 text-xs leading-5 text-white/75 sm:text-sm"><p>配送范围：[小区/街道名称] · 满 [X] 元起送 · [次日达/当日达]</p><p>[品牌/经营者名称]，自云南菌市直发 · 服务本地街坊 [X] 年</p></div>
    ...
    <section id="story" className="container py-16 sm:py-24"><div className="grid gap-9 md:grid-cols-2 md:items-center"><div><p className="text-xs font-semibold tracking-[.2em] text-forest-500">FROM MARKET TO TABLE</p><h2 className="mt-3 font-serif text-3xl leading-tight sm:text-4xl">一颗菌子的鲜味旅程</h2><p className="mt-6 text-sm leading-7 text-stone-600">云南的野生菌随着天气、海拔和菌季不断变化。</p><p className="mt-4 text-sm leading-7 text-stone-600">我们根据当天到货情况，查看菌子的完整度、新鲜度和品相，再选择适合家庭餐桌和餐饮使用的菌子。</p><p className="mt-4 text-sm leading-7 text-stone-600">这里不追求长期库存，而是跟随菌季，把当天状态更好的鲜菌分享给真正喜欢云南味道的人。</p></div><div className="grid grid-cols-2 gap-3"><div className="relative col-span-2 h-52 overflow-hidden rounded-2xl"><Image fill sizes="(max-width:768px) 100vw, 50vw" src="https://images.unsplash.com/photo-1448375240586-882707db888b?auto=format&fit=crop&w=1200&q=80" alt="云南山林" className="object-cover" /></div><div className="rounded-2xl bg-[#e8e7d8] p-5 text-sm text-forest-700">云南山林<br /><span className="text-xs text-stone-500">随菌季而来</span></div><div className="rounded-2xl bg-forest-900 p-5 text-sm text-white">市场鲜选<br /><span className="text-xs text-white/65">查看品相与状态</span></div></div></div></section>
    ...
    <footer className="bg-[#38281d] px-5 py-8 text-center text-xs leading-6 text-white/55">云南野生菌鲜选<br />昆明同城配送范围和当天库存请通过微信咨询<br />[经营者/店铺名称] · 联系电话：[占位] · 经营地址：[占位]<br />© 2026 菌鲜到</footer>
  </main>;
}
